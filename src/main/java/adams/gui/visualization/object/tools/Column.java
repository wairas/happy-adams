...
/**
 * Lets the user select a column (with specified padding) to use as an annotation.
 *
...
public class Column
  ...
  public String globalInfo() {
    return "Lets the user select a column (with specified padding) to use as an annotation.";
  }
  ...
  @Override
  public Icon getIcon() {
    return ImageManager.getIcon("column.png");
  }
  ...
  protected ToolMouseAdapter createMouseListener() {
	...
	if (MouseUtils.isLeftClick(e) && MouseUtils.hasNoModifierKey(e)) {
	  ...
	  int x = getCanvas().mouseToPixelLocation(e.getPoint()).x;
	  int index = AnnotationHelper.findLastIndex(getCanvas().getOwner().getReport(), m_Prefix) + 1;
          String current = m_Prefix + index;
          Rectangle rect = new Rectangle(x, 0, 1 + m_Padding*2, getCanvas().getImage().getHeight());
          getCanvas().getOwner().addUndoPoint("Adding column: " + rect);
  ...
  protected void addOptions(ParameterPanel paramPanel) {
    ...
    m_TextPadding.setToolTipText("The padding in pixels around the column");
